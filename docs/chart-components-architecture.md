...
> 💡 **용어 가이드**: 차트 관련 개발에서 사용되는 핵심 용어들에 대한 가이드는 [Chart Terminology Guide](./chart-terminology-guide.md)를 참조하세요.
...
```mermaid
graph TD
    A[MultiChartLayout] --> B[ChartDrawingToolbar]
    A --> C[ChartTopToolbar]
    A --> D[ChartGrid]
    A --> E[ChartSettingsDialog]
    A --> F[ChartIndicatorDialog]
...
| `selectedChartData` | `ChartData \| null` | 선택된 차트 데이터 |
...
- **레이아웃 드롭다운 관리**: `LayoutTemplateDropdown`을 포함하고, 이들의 표시 상태를 관리합니다.
...
const addChart = () => {
  const symbol = DEFAULT_FOREX_SYMBOLS[0];
