...
} from './TradingViewFeatures';
import type { TradingViewWidgetConfig, TradingViewWidget } from '@/types/tradingview';
import type { ChartConfig, ChartSymbolInfo, ChartSettings } from './ChartConfig';
import { generateThemeOverrides, getThemeFromSettings } from './ChartTheme';
...
export class ChartManager {
      ...
      this.waitForTradingView(() => {
        try {
          ...
          const widgetConfig: TradingViewWidgetConfig = {
            ...
            crosshair: {
              mode: config.settings?.scales.showCrosshair !== false ? 1 : 0, // 0: 비활성화, 1: 기본모드
              color: config.settings?.scales.crosshairColor || '#8f9299',
              width: 1,
              style: 0, // 0: 실선
            },
          };
  ...
  addIndicator(indicatorId: string, indicatorName: string): void {
    ...
    try {
      ...
      console.log('[ChartManager] Indicator added:', {
        indicatorId,
        indicatorName,
        inputs: studyInputs,
  ...
  addTestIndicators(): void {
    if (!this.widget || !this.isInitialized) {
      console.warn('[ChartManager] Chart not ready for adding test indicators');
      return;
    }

    try {
      console.log('[ChartManager] Adding test indicators for legend testing...');
      ...
      console.log('[ChartManager] Test indicators added for legend testing');
    } catch (error) {
      console.error('[ChartManager] Failed to add test indicators:', error);
    }
  }
