...
const validationRules = {
  englishFirstName: (value: string) => {
    if (!value || value.trim() === '') {
      return { isValid: false, message: '영문 이름을 입력해주세요.' };
    }
    if (!/^[A-Za-z\s]+$/.test(value)) {
      return { isValid: false, message: '영문만 입력 가능합니다.' };
    }
    return { isValid: true };
  },
  englishLastName: (value: string) => {
    if (!value || value.trim() === '') {
      return { isValid: false, message: '영문 성을 입력해주세요.' };
    }
    if (!/^[A-Za-z\s]+$/.test(value)) {
      return { isValid: false, message: '영문만 입력 가능합니다.' };
    }
  ...
  koreanAddress: (value: string) => {
    if (!value || value.trim() === '') {
      return { isValid: false, message: '한글 주소를 선택해주세요.' };
    }
    return { isValid: true };
  },
  koreanDetailAddress: (value: string) => {
    if (!value || value.trim() === '') {
      return { isValid: false, message: '한글 상세주소를 입력해주세요.' };
    }
    return { isValid: true };
  },
  englishAddress: (value: string) => {
    if (!value || value.trim() === '') {
      return { isValid: false, message: '영문 주소를 선택해주세요.' };
    }
    return { isValid: true };
  },
  englishDetailAddress: (value: string) => {
    if (!value || value.trim() === '') {
      return { isValid: false, message: '영문 상세주소를 입력해주세요.' };
    }
    return { isValid: true };
  },
};
