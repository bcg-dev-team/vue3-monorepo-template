...
export const apiKey: string =
  "50b312c44d36f6e5c3a72a852eff07a7f717e7264282270229c84c51693755b5";
 ...
 * @param fullSymbol - 전체 심볼 문자열 (예: "Bitfinex:BTC/USD" 또는 "BTC/USD")
 * @returns 파싱된 심볼 정보 또는 null
 */
export function parseFullSymbol(fullSymbol: string): ParsedSymbol | null {
  // 거래소:코인1/코인2 또는 코인1/코인2 모두 지원
  const match = fullSymbol.match(/^(?:(\w+):)?(\w+)\/(\w+)$/);
  if (!match) {
    return null;
  }
  return { exchange: "Bitfinex", fromSymbol: match[2], toSymbol: match[3] };
}
