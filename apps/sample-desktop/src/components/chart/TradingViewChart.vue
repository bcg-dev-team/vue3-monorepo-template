...
const showTradingButtons = computed(() => {
  ...
  return globalSettings.trading.showBuySellButtons;
});

// 모의 가격 데이터 (실제로는 실시간 데이터 연동)
...
const containerId = computed(() => {
  return props.chartId
    ? `tv_chart_container_${props.chartId}`
    : `tv_chart_container_${Math.random().toString(36).substr(2, 9)}`;
});
