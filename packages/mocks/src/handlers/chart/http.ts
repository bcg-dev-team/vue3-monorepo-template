...
import { SYMBOL_BASE_PRICES } from '@template/types';
...
function getBasePrice(fsym: string, tsym: string): number {
  ...
  if (SYMBOL_BASE_PRICES[exactSymbol]) {
    console.log(`[MSW] 정확한 심볼 매칭: ${exactSymbol} → ${SYMBOL_BASE_PRICES[exactSymbol]}`);
    return SYMBOL_BASE_PRICES[exactSymbol];
  }

  // 2. 부분 매칭 (우선순위 2) - 기존 호환성 유지
  if (SYMBOL_BASE_PRICES[fsym]) {
    console.log(`[MSW] 부분 심볼 매칭: ${fsym} → ${SYMBOL_BASE_PRICES[fsym]}`);
    return SYMBOL_BASE_PRICES[fsym];
  }
